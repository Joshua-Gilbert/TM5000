...
void execute_fft_with_config(void) {
    ...
    if (g_fft_config.peak_centering && peak_index > 0) {
        ...
        for (i = 0; i < g_fft_config.output_points && i < g_system->modules[target_slot].module_data_size; i++) {
            source_index = (i - shift_amount + g_fft_config.output_points) % g_fft_config.output_points;
    ...
    if (target_slot >= 0 && target_slot < 10) {
        ...
        if (g_fft_config.peak_centering && peak_index > 0) {
            /* Centered mode: peak at center, show frequency relative to peak */
            int center_position = g_fft_config.output_points / 2;
            g_traces[target_slot].x_offset = -center_position * freq_resolution;
        } else {
