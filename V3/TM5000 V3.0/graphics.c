...
/* Snap graph scale to clean engineering values to prevent precision errors */
void snap_graph_scale_to_clean_values(void) {
    float y_range, center, per_div, clean_per_div;
    ...
    if (per_div <= 0.0) return;  /* Safety check */
    
    /* Find clean engineering per-division value */
    magnitude = log10(fabs(per_div));
    exponent = (int)floor(magnitude);
    normalized = per_div / pow(10.0, exponent);
    
    /* Round to engineering values: 1, 2, 5 */
    if (normalized <= 1.5) {
        clean_per_div = 1.0 * pow(10.0, exponent);
    } else if (normalized <= 3.0) {
        clean_per_div = 2.0 * pow(10.0, exponent);
    } else if (normalized <= 7.0) {
        clean_per_div = 5.0 * pow(10.0, exponent);
    } else {
        clean_per_div = 10.0 * pow(10.0, exponent);
    }
    
    /* Set clean range (5 divisions for screen) */
    y_range = clean_per_div * 5.0;
    g_graph_scale.min_value = center - y_range / 2.0;
    g_graph_scale.max_value = center + y_range / 2.0;
}
...
void draw_grid_dynamic(int max_samples) {
    ...
    /* Simple unit switching based on range */
    ...
    if (unit_label != NULL) {
        fill_rectangle(2, 2, 25, 10, 0);  /* Clear area with black */
        draw_text_scaled(2, 2, unit_label, 3, 1, 1);
    }
