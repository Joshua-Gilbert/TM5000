...
void load_settings(void) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strcmp(line, "[ControlPanel]") == 0) {
            ...
            /* Clear existing modules and free buffers */
            for (slot = 0; slot < 10; slot++) {
                free_module_buffer(slot);  /* Free any existing buffers */
                g_system->modules[slot].enabled = 0;
                g_system->modules[slot].module_type = MOD_NONE;
                g_system->modules[slot].gpib_address = 0;
                g_system->modules[slot].description[0] = '\0';
            ...
            while (fgets(line, sizeof(line), fp)) {
                ...
                if (sscanf(line, "Module%d=%d,%d,%[^\n]", &slot, &type, &addr, desc) == 4) {
                    if (slot >= 0 && slot < 10) {
                        ...
                        /* CRITICAL: Allocate data buffer for loaded module */
                        if (!allocate_module_buffer(slot, 1000)) {
                            printf("WARNING: Failed to allocate buffer for slot %d\n", slot);
                        } else {
                            printf("Module S%d (%s) loaded and ready\n", slot, desc);
                        }
                    }
                }
            }
            
            /* Immediately sync loaded modules with traces for display */
            sync_traces_with_modules();
            {
                int enabled_count = 0;
                for (slot = 0; slot < 10; slot++) {
                    if (g_system->modules[slot].enabled) enabled_count++;
                }
                printf("Module loading complete - %d modules configured\n", enabled_count);
            }
    ...
    /* Note: Module initialization and buffer allocation will happen 
     * when modules are actually configured or used, not during loading 
     * This matches the behavior of the working TM5000L.c version */
