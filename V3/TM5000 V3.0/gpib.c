...
void gpib_remote_dm5120(int address) {
    ...
    sprintf(cmd_buffer, "remote %d%s", address, termination);
    ieee_write(cmd_buffer);
...
void gpib_local_dm5120(int address) {
    ...
    sprintf(cmd_buffer, "local %d%s", address, termination);
    ieee_write(cmd_buffer);
    delay(100);
}
...
void gpib_clear_dm5120(int address) {
    ...
    sprintf(cmd_buffer, "clear %d%s", address, termination);
    ieee_write(cmd_buffer);
    delay(200);
}
...
void send_custom_command(void) {
    ...
    printf("Enter GPIB address (10-30): ");
    scanf("%d", &address);
