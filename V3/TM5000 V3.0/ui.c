        ...
        if (g_mouse.present) {
            while (choice == 0) {
                ...
                if (g_mouse.left_button) {
                    ...
                    if (g_mouse.y == 6) { choice = '1'; break; }   /* Single Measurement */
                    if (g_mouse.y == 8) { choice = '2'; break; }   /* Continuous Measurement */
                    if (g_mouse.y == 10) { choice = '3'; break; }  /* Graph Display */
                    if (g_mouse.y == 12) { choice = '4'; break; }  /* Math Functions */
                    if (g_mouse.y == 14) { choice = '0'; break; }  /* Return to Main Menu */
                }
            ...
            if (selected_trace >= 0 && g_traces[selected_trace].enabled) {
                ...
                if (is_fft_trace[selected_trace]) {
                    ...
                    if (g_traces[selected_trace].data_count > 1) {
                        freq = (sample_rate / 2.0) * sample_num / (g_traces[selected_trace].data_count - 1);
                    }
                    sprintf(readout, "S%d[%.1f%s]:%.3f", selected_trace, 
                            freq / freq_multiplier, freq_unit_str, value);
                } else {
                    /* Display slot number and exact voltage value (no units) */
                    float display_value = value;
                    float current_range = g_graph_scale.max_value - g_graph_scale.min_value;
                    
                    /* Scale the display value based on current range */
                    if (current_range < 0.001) {
                        /* µV range - show as microvolts */
                        display_value = value * 1000000.0;
                        sprintf(readout, "%d:%.0f", sample_num, display_value);
                    } else if (current_range < 1.0) {
                        /* mV range - show as millivolts */
                        display_value = value * 1000.0;
                        sprintf(readout, "%d:%.1f", sample_num, display_value);
                    } else {
                        /* V range - show as volts */
                        sprintf(readout, "%d:%.4f", sample_num, display_value);
                    }
                }
        ...
        if (kbhit()) {
            ...
            switch(toupper(key)) {
                ...
                case '+':  /* Zoom in */
                    ...
                    /* Clean up precision errors only when units per div don't match */
                    {
                        float current_range = g_graph_scale.max_value - g_graph_scale.min_value;
                        float per_div = current_range / 5.0;
                        float expected_range = per_div * 5.0;
                        float range_error = fabs(current_range - expected_range);
                        
                        zoom_operations_count++;
                        
                        if (current_range < 1.0) {
                            /* For mV ranges: check for precision drift more frequently */
                            if (range_error > (current_range * 0.000001) && zoom_operations_count >= 2) {
                                snap_graph_scale_to_clean_values();
                                zoom_operations_count = 0;
                            }
                        } else {
                            /* For V ranges: check for precision drift less frequently */
                            if ((range_error > (current_range * 0.000001) && zoom_operations_count >= 3) ||
                                zoom_operations_count >= 8) {
                                snap_graph_scale_to_clean_values();
                                zoom_operations_count = 0;
                            }
                        }
