...
void draw_grid_dynamic(int max_samples) {
    ...
    /* Unit priority: dB > frequency > voltage */
    if (has_db_traces) {
    ...
    for (i = 0; i <= 5; i++) {
        ...
        if (pos >= GRAPH_TOP && pos <= GRAPH_BOTTOM) {
            ...
            if (has_db_traces && scale_multiplier == 1.0 && strcmp(unit_label, "DB") == 0) {
                /* For dB, show limited precision and only on major grid lines */
                if (i % 2 == 0) {  /* Only show labels on major grid lines (every other line) */
                    sprintf(label, "%.0fdB", value);
                } else {
