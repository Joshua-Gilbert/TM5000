...
#include "print.h"
#include "graphics.h"  /* For get_graph_units() function */
...
/* Print-specific unit conversion for PostScript glyphs */
void get_print_units(float range, char **unit_str, float *scale_factor, int *decimal_places, char **postscript_unit) {
    ...
    if (strcmp(*unit_str, "UV") == 0) {
        ...
        *postscript_unit = *unit_str;  /* Default passthrough for future units like MA, etc. */
    }
}
        ...
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            ...
            
            {
                ...
                
                {
                    ...
                    get_print_units(fabs(last_val), &last_unit, &last_scale, &last_dec, &last_ps_unit);
                    get_print_units(fabs(min_trace), &min_unit, &min_scale, &min_dec, &min_ps_unit);
                    get_print_units(fabs(max_trace), &max_unit, &max_scale, &max_dec, &max_ps_unit);
                ...
                
                {
                    ...
                    
                    {
                        ...
                        get_print_units(trace_range, &range_unit, &range_scale, &range_dec, &range_ps_unit);
                        get_print_units(fabs(avg_val), &avg_unit, &avg_scale, &avg_dec, &avg_ps_unit);
                        get_print_units(rms_val, &rms_unit, &rms_scale, &rms_dec, &rms_ps_unit);
...
void print_graph_postscript(void) {
    ...
    printf("\nEnter graph title (or press Enter for default): ");
    fflush(stdout);
    {
        char graph_title[256];
        if (fgets(graph_title, sizeof(graph_title), stdin) && graph_title[0] != '\n') {
            graph_title[strcspn(graph_title, "\n")] = '\0';
        } else {
