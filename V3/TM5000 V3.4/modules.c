...
void configure_fg5010(int slot) {
    ...
    while (!done) {
        ...
        printf("  6. Duty Cycle: %.1f%% (Square/Pulse only)\n", cfg->duty_cycle);
        ...
        switch (choice) {
            case 1:
                printf("Enter frequency in Hz (0.1 to 15000000): ");
                scanf("%s", input);
                cfg->frequency = atof(input);
                if (cfg->frequency < 0.1) cfg->frequency = 0.1;
                if (cfg->frequency > 15000000.0) cfg->frequency = 15000000.0;
                break;
                
            case 2:
                printf("Enter amplitude in Vpp (0.01 to 20.0): ");
                scanf("%s", input);
            ...
            case 3:
                printf("Enter DC offset in V (-10.0 to +10.0): ");
                scanf("%s", input);
                cfg->offset = atof(input);
                if (cfg->offset < -10.0) cfg->offset = -10.0;
                if (cfg->offset > 10.0) cfg->offset = 10.0;
                break;
            ...
            case 6:
                printf("Enter duty cycle %% (1.0 to 99.0): ");
                scanf("%s", input);
                cfg->duty_cycle = atof(input);
                if (cfg->duty_cycle < 1.0) cfg->duty_cycle = 1.0;
                if (cfg->duty_cycle > 99.0) cfg->duty_cycle = 99.0;
                break;
...
void configure_fg5010_advanced(int slot) {
    ...
    while (!done) {
        ...
        switch (choice) {
            ...
            case 10:
                if (strcmp(cfg->mod_type, "AM") == 0) {
                    strcpy(cfg->mod_type, "FM");
                } else {
                    strcpy(cfg->mod_type, "AM");
                }
...
void fg5010_set_sweep(int address, int enable, float start_freq, float stop_freq, float time) {
    char cmd[100];
    if (enable) {
        sprintf(cmd, "SWE:STAT ON;SWE:STAR %.3f;SWE:STOP %.3f;SWE:TIME %.3f", 
                start_freq, stop_freq, time);
    } else {
        strcpy(cmd, "SWE:STAT OFF");
    }
    gpib_write(address, cmd);
    delay(50);
}
...
void fg5010_set_trigger(int address, char *source, char *slope, float level) {
    char cmd[100];
    sprintf(cmd, "TRIG:SOUR %s;TRIG:SLOP %s;TRIG:LEV %.2f", source, slope, level);
    gpib_write(address, cmd);
    delay(50);
}
...
void fg5010_set_modulation(int address, int enable, char *type, float freq, float depth) {
    char cmd[100];
    if (enable) {
        sprintf(cmd, "MOD:STAT ON;MOD:TYP %s;MOD:FREQ %.2f;MOD:DEPT %.1f", 
                type, freq, depth);
    } else {
        strcpy(cmd, "MOD:STAT OFF");
    }
    gpib_write(address, cmd);
    delay(50);
}
...
void fg5010_set_burst(int address, int enable, int count, float period) {
    char cmd[100];
    if (enable) {
        sprintf(cmd, "BURS:STAT ON;BURS:NCYC %d;BURS:INT:PER %.3f", count, period);
    } else {
        strcpy(cmd, "BURS:STAT OFF");
    }
    gpib_write(address, cmd);
    delay(50);
}
...
int fg5010_read_output_status(int address) {
    ...
    delay(100);
    if (gpib_read(address, buffer, sizeof(buffer)) > 0) {
        return (strstr(buffer, "ON") != NULL || strstr(buffer, "1") != NULL) ? 1 : 0;
    }
    return 0;
}
...
void init_dm5120_config_enhanced(int slot) {
    ...
    g_dm5120_config[slot].buffer_size = MAX_SAMPLES_PER_MODULE;
...
/* Calculate timing delay based on DM5120 digit setting and operation type */
int dm5120_calculate_delay(int slot, int operation_type, int sample_count) {
    dm5120_config *cfg = &g_dm5120_config[slot];
    int base_delay_ms;
    
    /* Base delay per measurement based on digit setting */
    switch (cfg->digits) {
        case 3:
        case 4:
            base_delay_ms = 3;    /* ~300 readings/sec */
            break;
        case 5:
            base_delay_ms = 10;   /* ~100 readings/sec */
            break;
        case 6:
        default:
            base_delay_ms = 50;   /* ~20 readings/sec for 6-digit precision */
            break;
    }
    ...
    switch (operation_type) {
        ...
        case 1: /* Buffer setup */
            return base_delay_ms * 2;  /* Setup overhead */
        case 2: /* Buffer data retrieval */
            return (base_delay_ms * sample_count) + 100;  /* Scale with sample count + overhead */
        case 3: /* Buffer start */
            return base_delay_ms;
        default:
            return base_delay_ms;
...
void dm5120_enable_buffering(int address, int buffer_size) {
    char cmd[50];
    int slot = 0; /* TODO: Pass slot parameter properly */
...
void dm5120_start_buffer_sequence(int address, int slot) {
    gpib_write_dm5120(address, "BUFSTART");
    
    /* Use digit-aware timing for buffer start */
    delay(dm5120_calculate_delay(slot, 3, 1));
}
...
int dm5120_fill_buffer_complete(int address, int slot, float far *buffer, int buffer_size) {
    ...
    dm5120_enable_buffering(address, buffer_size);
    
    /* Step 2: Start buffer sequence */
    dm5120_start_buffer_sequence(address, slot);
...
void test_dm5120_comm_debug(int address) {
    ...
    sprintf(buffer, "output %2d; INIT\r\n", address);
    printf("   Command: %s", buffer);
    ieee_write(buffer);
    delay(500);
    
    printf("\n2. Sending function setup:\n");
    sprintf(buffer, "output %2d; FUNCT DCV\r\n", address);
    printf("   Command: %s", buffer);
    ieee_write(buffer);
    delay(200);
    
    printf("\n3. Requesting reading:\n");
    sprintf(buffer, "output %2d; voltage?\r\n", address);
    printf("   Command: %s", buffer);
    ieee_write(buffer);
    delay(200);
    
    printf("\n4. Reading response:\n");
    sprintf(buffer, "enter %2d\r\n", address);
    printf("   Command: %s", buffer);
    ieee_write(buffer);
    delay(200);
