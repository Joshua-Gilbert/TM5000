 ...
 * Version 2.3 - Per-Module Data Storage and Compact Display
...
/* Draw complete grid with scale - now with automatic mV switching */
void draw_grid(void) {
    int i, x, y;
    float value;
    int pos;
    ...
    if (y_range < 0.1) {  /* Less than 100mV range */
        use_millivolts = 1;
        scale_multiplier = 1000.0;
        unit_label = "mV";
    }
    
    /* Draw border with double lines for enhanced look */
    ...
    for (i = 0; i <= 5; i++) {
        ...
        if (pos >= GRAPH_TOP && pos <= GRAPH_BOTTOM) {
            /* Draw the grid line with different pattern for major lines */
            if (i % 2 == 0) {
                /* Major grid line - solid dots */
                for (x = GRAPH_LEFT; x <= GRAPH_RIGHT; x += 3) {
                    plot_pixel(x, pos, 1);
                    plot_pixel(x+1, pos, 1);
                }
            } else {
                /* Minor grid line - sparse dots */
                for (x = GRAPH_LEFT; x <= GRAPH_RIGHT; x += 6) {
                    plot_pixel(x, pos, 1);
                }
            }
            
            /* Draw the label with unit conversion */
            if (use_millivolts) {
                sprintf(label, "%.1f", value * scale_multiplier);
            } else {
                /* For volts, show more precision if range is small */
                if (y_range < 1.0) {
...
void graph_display(void) {
    ...
    while (!done) {
        if (need_redraw) {
            ...
            for (i = 0; i < 8; i++) {
                if (g_traces[i].enabled && g_traces[i].data_count > 1) {
                    ...
                    for (j = 0; j < g_traces[i].data_count - 1; j++) {
                        ...
                        draw_line_aa(x1, y1, x2, y2, g_traces[i].color);
                    }
                }
            }
            ...
            /* Show current scale range on right */
...
void main_menu(void) {
    ...
    while (!done) {
        clrscr();
        gotoxy(20, 1);
        printf("TM5000 CONTROL SYSTEM v2.3");
        if (g_mouse.present) {
            gotoxy(50, 1);
            printf("[Mouse: ON]");
        }
...
int main(void) {
    ...
    clrscr();
    printf("TM5000 GPIB Control System v2.3\n");
