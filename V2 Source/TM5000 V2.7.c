...
void print_graph_pcl(void) {
    ...
    /* Determine units and scaling - same logic as PostScript */
    if (y_range < 0.000001) {
        unit_str = "uV";  /* microvolts */
        scale_factor = 1000000.0;
        decimal_places = 1;  /* Show 0.1 µV resolution max */
...
void print_graph_postscript(void) {
    ...
    if (y_range < 0.000001) {
        unit_str = "uV";  /* Use plain text for now, will handle in PostScript */
        scale_factor = 1000000.0;
        decimal_places = 1;  /* Show 0.1 µV resolution max */
    } else if (y_range < 0.001) {
        unit_str = "uV";
    ...
    print_string("%%Creator: TM5000 Control System v2.7\r\n");
    ...
    if (decimal_places == 0) {
        sprintf(label, "(Range: %.0f ) show ", y_range * scale_factor);
    } else {
        sprintf(label, "(Range: %.*f ) show ", decimal_places + 1, y_range * scale_factor);
    }
    print_string(label);
    /* Handle units */
    if (strcmp(unit_str, "uV") == 0) {
        ...
        sprintf(label, "(%s, Sample Rate: ) show ", unit_str);
        print_string(label);
    }
    ...
    for (i = 0; i < 8; i++) {
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            ...
            if (decimal_places == 0) {
                sprintf(label, "(%.0f) show ", min_trace * scale_factor);
            } else {
                sprintf(label, "(%.*f) show ", decimal_places, min_trace * scale_factor);
            }
            print_string(label);
            if (strcmp(unit_str, "uV") == 0) {
                print_string("(");
                lpt_send_byte(0xB5);  /* µ character */
                print_string("V, Max: ) show ");
            } else {
                sprintf(label, "(%s, Max: ) show ", unit_str);
                print_string(label);
            }
            
            if (decimal_places == 0) {
                sprintf(label, "(%.0f) show ", max_trace * scale_factor);
            } else {
                sprintf(label, "(%.*f) show ", decimal_places, max_trace * scale_factor);
            }
            print_string(label);
            if (strcmp(unit_str, "uV") == 0) {
                print_string("( ) show /mu glyphshow (V, Range: ) show ");
            } else {
                sprintf(label, "(%s, Range: ) show ", unit_str);
                print_string(label);
            }
            
            if (decimal_places == 0) {
                sprintf(label, "(%.0f) show ", (max_trace - min_trace) * scale_factor);
            } else {
                sprintf(label, "(%.*f) show ", decimal_places + 1, (max_trace - min_trace) * scale_factor);
            }
            print_string(label);
            if (strcmp(unit_str, "uV") == 0) {
...
void main_menu(void) {
    ...
    while (!done) {
        ...
        printf("\n\n");
        printf("          TM5000 CONTROL SYSTEM v2.7\n");
        printf("          ==========================\n\n");
...
int main(void) {
    ...
    clrscr();
    printf("TM5000 GPIB Control System v2.7\n");
