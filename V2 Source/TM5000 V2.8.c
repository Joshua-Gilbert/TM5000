 ...
 * 
 * Changes in v2.8:
 * - Expanded graph display to support up to 1000 samples
 * - Enhanced X-axis compression for high sample counts
 * - Optimized 287 coprocessor usage for large datasets
 * - Improved memory management for extended sampling
 * - Dynamic sample decimation for smooth display
 * - Graph configuration menu (H key) with sample range selection
 * - Simplified graph display instructions (reduced clutter)
 * - Sample range selector: display all or custom range (1-1000)
 * - Enhanced zoom quantization: 0.1mV steps for ranges < 5mV
 * - Fixed zoom limitation preventing ranges smaller than 4mV
 * - Status indicators for sample range (R:100-200)
 * - Status indicators showing sample count and decimation state
 * - Added LF termination option for DM5120 instruments showing "LF" address
 * - Fixed DM5120 error 101 communication issue with Line Feed mode support
 * - Enhanced printing with custom headers and comprehensive statistics
 * - Fixed Range display unit formatting (was showing "401 microV" incorrectly)
 * - Fixed legend min/max unit inconsistency (proper per-value unit selection)
 * - Added measurement duration, timestamp, and trace statistics to printouts
 * - Fixed loaded measurements not appearing in graph menu and math calculations
 * - Added sync_traces_with_modules() function for proper data synchronization
 * - Fixed Brother PostScript printing: corrected µ symbol corruption in legends
 * - Fixed Brother PostScript printing: per-value unit determination for Min/Max/Range
 * - Fixed Brother PostScript printing: proper range display unit calculation
 *
 * Changes in v2.7:
 * - High-precision measurement display with automatic µV/mV/V unit selection
 * - Enhanced print functions supporting microvolts without decimals
 * - Brother HL5370DL native PostScript (BR-Script 3) printing support
 * - Fixed text/PCL printing for proper graph scaling on 8.5x11" paper
 * - Dynamic unit conversion in graph display matching print output
 * - Improved Y-axis labeling for sub-millivolt measurements
 * - Automatic decimal place adjustment based on measurement range
 * - Graph readout shows proper units in cursor display
 * - Print functions preserve exact zoom and scale settings
 * - 50-line resolution for text printing (increased from 40)
 *
 * Changes in v2.6:
...
void gpib_write_dm5120(int address, char *command) {
    ...
    /* Find slot for this address to check LF termination setting */
    slot = -1;
    {
        int i;
        for (i = 0; i < 8; i++) {
    ...
    /* Select termination based on module configuration */
    ...
    /* Format command with space after semicolon and selected termination */
...
int gpib_read_dm5120(int address, char *buffer, int maxlen) {
    ...
    /* Use ENTER command with selected termination */
...
void configure_modules(void) {
    ...
    if (slot >= 0 && slot < 8) {
        ...
        if (type >= 0 && type <= 7) {
            ...
            if (type != MOD_NONE) {
                /* Determine default address for prompt */
                int default_addr;
                switch(type) {
                    case MOD_DM5120:
                        default_addr = 16;
                ...
                do {
                    ...
                    if (sscanf(buffer, "%d", &addr) == 1) {
                        ...
                        /* Set module-specific default addresses */
                        switch(type) {
                            case MOD_DM5120:
                                addr = 16;
                                break;
                ...
                if (type == MOD_DM5120) {
                    ...
                    /* Ask user about LF termination for DM5120 */
...
void get_graph_units(float range, char **unit_str, float *scale_factor, int *decimal_places) {
    if (range < 0.000001) {
        ...
        *decimal_places = 1;
        ...
        /* Switch to mV for better precision when V would need >2 decimals */
        *unit_str = "mV";
        *scale_factor = 1000.0;
        *decimal_places = 0;
    } else {
        ...
        if (need_redraw) {
            ...
            for (i = 0; i < 8; i++) {
                if (g_traces[i].enabled && g_traces[i].data_count > 1) {
                    /* Calculate proper scaling and decimation for 1000 samples */
                    ...
                    /* For datasets > GRAPH_WIDTH, use decimation to maintain smooth display */
                    if (display_count > GRAPH_WIDTH) {
                        if (g_has_287) {
                            /* 287: Fast calculation */
                            decimation_factor = (display_count + GRAPH_WIDTH - 1) / GRAPH_WIDTH;
                        } else {
                            /* Software: Use integer math */
                    ...
                    /* Draw waveform with decimation for large datasets */
                    for (j = 0; j < effective_samples - 1; j++) {
                        ...
                        /* Get Y positions with 287 optimization using decimated samples */
                        if (g_has_287) {
                            /* 287: Use pre-calculated scale factor */
            ...
            draw_text(2, 195, "0-7:Select SHIFT+#:Toggle ESC:Exit", 3);
        ...
        if (kbhit()) {
            ...
            switch(toupper(key)) {
                ...
                case '+':  /* Zoom in */
                    ...
                    /* Quantize range for microvolt scales - minimum 5µV increments */
                    if (range < 0.000050) {  /* Less than 50µV total range */
                        /* Round range to nearest 5µV increment */
                    ...
                    /* Quantize range for sub-millivolt scales - 0.1mV increments */
                    else if (range < 0.005) {  /* Less than 5mV total range */
                        /* Round range to nearest 0.1mV increment */
                    ...
                    /* Quantize range for millivolt scales - 1mV increments */
                    else if (range < 0.050) {  /* Less than 50mV total range */
                        /* Round range to nearest 1mV increment */
                        float range_mv = range * 1000.0;  /* Convert to mV */
...
void print_graph_menu(void) {
    ...
    printf("4. Tektronix 4662 Plotter\n");
	printf("5. Cancel\n\n");
...
/* High-precision text-based graph printing with µV support */
void print_graph_text(void) {
    int i, j, x, y;
    char line[81];
    char plot_chars[80];
    ...
    /* Use centralized unit determination for consistency */
    get_graph_units(y_range, &unit_str, &scale_factor, &decimal_places);
    
    /* Convert µ to 'u' for ASCII text printing compatibility */
    ...
    /* Print header with custom text option */
    print_string("TM5000 HIGH-PRECISION MEASUREMENT GRAPH\r\n");
    print_string("=======================================\r\n");
    
    /* Ask for custom header */
    ...
    fflush(stdout);
    {
        ...
        if (fgets(custom_header, sizeof(custom_header), stdin) && custom_header[0] != '\n') {
            /* Remove newline and add to print */
    ...
    /* Calculate range with proper unit selection */
    {
        char *range_unit_str;
    ...
    /* Print additional measurement statistics */
    ...
    for (i = 0; i < 8; i++) {
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            ...
            /* Print statistics with high precision and per-value unit determination */
            {
                ...
                /* Get units for each value individually for consistency */
                ...
                /* Convert µ to 'u' for ASCII compatibility */
                ...
                /* Additional statistics */
                {
                    ...
                    /* Get units for additional statistics */
...
void print_graph_pcl(void) {
    ...
    int graph_x = 100;      /* Left margin in dots */
    int graph_y = 200;      /* Top margin in dots */
    int graph_width = 400;  /* Graph width in dots */
    int graph_height = 400; /* Graph height in dots */
    char *unit_str;
    float scale_factor;
    int decimal_places;
    float min_val, max_val;
    ...
    /* Print scale info with proper formatting */
    print_string("\x1B*p200x140Y");
    print_string("\x1B(s1p10v0s0b0T");  /* 10 point font */
    if (decimal_places == 0) {
        sprintf(cmd, "Scale: %.0f to %.0f %s", 
                min_val * scale_factor,
                max_val * scale_factor,
                unit_str);
    } else {
        sprintf(cmd, "Scale: %.*f to %.*f %s", 
                decimal_places, min_val * scale_factor,
                decimal_places, max_val * scale_factor,
                unit_str);
    }
    ...
    /* Print range and sample rate info */
    print_string("\x1B*p200x160Y");
    if (decimal_places == 0) {
        sprintf(cmd, "Range: %.0f %s, Sample Rate: %d ms", 
                y_range * scale_factor, unit_str, g_control_panel.sample_rate_ms);
    } else {
        sprintf(cmd, "Range: %.*f %s, Sample Rate: %d ms", 
                decimal_places + 1, y_range * scale_factor, 
                unit_str, g_control_panel.sample_rate_ms);
    }
    print_string(cmd);
    
    /* Start raster graphics at position graph_x, graph_y */
    sprintf(cmd, "\x1B*p%dx%dY", graph_x, graph_y);
    print_string(cmd);
    ...
    for (y = 0; y < graph_height; y++) {
        ...
        /* Draw border - enhanced with double lines at edges */
        if (y == 0 || y == 1) {
            /* Top border - double line */
            for (x = 0; x < graph_width; x++) {
                print_buffer[x / 8] |= (0x80 >> (x % 8));
            }
        } else if (y == graph_height - 1 || y == graph_height - 2) {
        ...
        /* Left border - double line */
        print_buffer[0] |= 0xC0;  /* Two leftmost pixels */
        
        /* Right border - double line at pixel 399-400 */
        print_buffer[49] |= 0x06;  /* Two rightmost pixels of 400-pixel width */
        
        /* Draw grid lines - 10 divisions for high precision */
        if (y > 0 && y < graph_height - 1 && (y % (graph_height / 10)) == 0) {
            /* Horizontal grid - dashed pattern */
            for (x = 2; x < graph_width - 2; x += 6) {
                print_buffer[x / 8] |= (0x80 >> (x % 8));
                if (x + 1 < graph_width - 2) {
                    print_buffer[(x + 1) / 8] |= (0x80 >> ((x + 1) % 8));
                }
            }
        }
        ...
        /* Vertical grid lines */
        for (i = 1; i < 10; i++) {
            x_pos = (graph_width * i) / 10;
            if ((y % 6) < 2) {  /* Dashed pattern */
                print_buffer[x_pos / 8] |= (0x80 >> (x_pos % 8));
            }
        }
        ...
        for (i = 0; i < 8; i++) {
            if (g_traces[i].enabled && g_traces[i].data_count > 0) {
                /* Calculate scales */
                x_scale = (float)(graph_width - 4) / (float)(g_traces[i].data_count - 1);
                y_scale = (float)(graph_height - 4) / y_range;
                
                /* Map samples across the width */
                for (x = 2; x < graph_width - 2; x++) {
                    ...
                    y_pos = (int)((graph_height - 4) - (value - min_val) * y_scale);
                    
                    /* Plot with different patterns for different traces */
                    if (y_pos == y - 2) {
                        /* Main point */
                        print_buffer[x / 8] |= (0x80 >> (x % 8));
                        
                        /* Add thickness for selected trace */
                        if (i == 0 && y > 2 && y < graph_height - 2) {
                            /* Thicker line for primary trace */
                            print_buffer[x / 8] |= (0x80 >> (x % 8));
                            if (x > 0) {
                                print_buffer[(x-1) / 8] |= (0x80 >> ((x-1) % 8));
                            }
        ...
        /* Send raster line */
        sprintf(cmd, "\x1B*b%dW", 50);  /* 50 bytes = 400 pixels */
        print_string(cmd);
    ...
    /* Print Y-axis labels with high precision */
    print_string("\x1B(s1p8v0s0b0T");  /* 8 point font */
    for (i = 0; i <= 10; i++) {
        y_label_pos = graph_y + (graph_height * i / 10);
        y_value = max_val - (y_range * i / 10.0);
        
        sprintf(cmd, "\x1B*p20x%dY", y_label_pos - 4);
        print_string(cmd);
        
        /* Format with appropriate precision */
        if (decimal_places == 0) {
            sprintf(label, "%7.0f", y_value * scale_factor);
        } else {
            sprintf(label, "%7.*f", decimal_places, y_value * scale_factor);
        }
        print_string(label);
    }
    
    /* Y-axis unit label */
    sprintf(cmd, "\x1B*p5x%dY", graph_y + graph_height/2);
    print_string(cmd);
    print_string(unit_str);
    
    /* Print X-axis labels */
    for (i = 0; i <= 10; i++) {
        x_label_pos = graph_x + (graph_width * i / 10);
        if (i % 2 == 0) {  /* Label every other tick */
            sprintf(cmd, "\x1B*p%dx%dY", x_label_pos - 10, graph_y + graph_height + 15);
            print_string(cmd);
            sprintf(cmd, "%d", (max_samples * i) / 10);
            print_string(cmd);
        }
    }
    
    /* X-axis label */
    sprintf(cmd, "\x1B*p%dx%dY", graph_x + graph_width/2 - 40, graph_y + graph_height + 30);
    print_string(cmd);
    print_string("Sample Number");
    
    /* Enhanced Legend with statistics */
    print_string("\x1B*p100x650Y");
    print_string("\x1B(s1p10v0s0b0T");  /* 10 point font */
    print_string("Legend:");
    ...
    for (i = 0; i < 8; i++) {
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            ...
            sprintf(cmd, "\x1B*p140x%dY", y + 15);
            print_string(cmd);
            print_string("\x1B(s1p8v0s0b0T");  /* 8 point font for stats */
            
            /* Format statistics with appropriate precision */
            if (decimal_places == 0) {
                sprintf(cmd, "Min: %.0f %s, Max: %.0f %s, Range: %.0f %s",
                        min_trace * scale_factor, unit_str,
                        max_trace * scale_factor, unit_str,
                        (max_trace - min_trace) * scale_factor, unit_str);
            } else {
                sprintf(cmd, "Min: %.*f %s, Max: %.*f %s, Range: %.*f %s",
            ...
            print_string("\x1B(s1p10v0s0b0T");  /* Back to 10 point */
            y += 35;
            
            if (y > 770) break;  /* Don't overflow page */
        }
    }
    
    /* Print measurement info */
    print_string("\x1B*p100x820Y");
    print_string("\x1B(s1p8v0s0b0T");  /* 8 point font */
    sprintf(cmd, "Graph Settings: Zoom %.2fx, Auto-scale: %s", 
            g_graph_scale.zoom_factor,
            g_graph_scale.auto_scale ? "ON" : "OFF");
    print_string(cmd);
    ...
    printf("Enhanced PCL graph printed successfully.\n");
    printf("Press any key to continue...");
    getch();
}
/*PostScript printing function for Brother HL5370DW */
void print_graph_postscript(void) {
    int i, j, x, y;
    float value, y_range;
    char label[100];
    ...
    print_string("%%Creator: TM5000 Control System v2.8\r\n");
    ...
    /* PostScript prolog for BR-Script 3 compatibility */
    ...
    print_string(label);
    /* Handle units separately for proper µ symbol */
    if (strcmp(unit_str, "uV") == 0) {
        /* Use glyphshow method - most reliable for BR-Script 3 */
        print_string("( ) show\r\n");
    ...
    print_string(label);
    /* REMOVED duplicate print_string(label) here - this was the bug! */
    ...
    for (i = 0; i < 8; i++) {
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            ...
            /* Statistics on next line with per-value unit determination */
            print_string("/Times-Roman findfont 8 scalefont setfont\r\n");
            sprintf(label, "120 %d moveto\r\n", y - 10);
            print_string(label);
            {
...
/* Tektronix 4662 Plotter Function for TM5000 */

/* Tektronix 4010/4014 command constants */
#define TEK_GS    0x1D    /* Enter graphics mode */
#define TEK_US    0x1F    /* Enter alpha (text) mode */
#define TEK_FS    0x1C    /* Enter point plot mode */
#define TEK_RS    0x1E    /* Enter incremental plot mode */
#define TEK_ESC   0x1B    /* Escape */
#define TEK_FF    0x0C    /* Form feed */
#define TEK_CR    0x0D    /* Carriage return */
#define TEK_LF    0x0A    /* Line feed */

/* Function to send commands to Tek 4662 via IOTech driver */
void tek4662_send(int address, const char *data, int length) {
    char cmd[256];
    int i;
    
    /* Format for IOTech driver */
    sprintf(cmd, "output %d;", address);
    ieee_write(cmd);
    
    /* Send data in chunks to avoid buffer overflow */
    for (i = 0; i < length; i++) {
        char byte_cmd[10];
        sprintf(byte_cmd, "%c", data[i]);
        ieee_write(byte_cmd);
    }
    
    /* Add CR+LF as configured */
    ieee_write("\r\n");
    delay(50);
}

/* Send Tektronix coordinate */
void tek4662_send_coord(int address, int x, int y) {
    unsigned char coord[4];
    
    /* Tektronix coordinate encoding:
     * - 10-bit values (0-1023)
     * - Split into 5-bit high and low components
     * - Add offset for proper ASCII range */
    coord[0] = 0x20 | ((y >> 5) & 0x1F);    /* High Y */
    coord[1] = 0x60 | (y & 0x1F);           /* Low Y */
    coord[2] = 0x20 | ((x >> 5) & 0x1F);    /* High X */
    coord[3] = 0x40 | (x & 0x1F);           /* Low X */
    
    tek4662_send(address, (char*)coord, 4);
}

/* Move to position (pen up) */
void tek4662_move_to(int address, int x, int y) {
    char gs = TEK_GS;
    tek4662_send(address, &gs, 1);  /* Enter graphics mode */
    tek4662_send_coord(address, x, y);  /* First coord is always move */
}

/* Draw to position (pen down) */
void tek4662_draw_to(int address, int x, int y) {
    tek4662_send_coord(address, x, y);  /* Subsequent coords draw */
}

/* Draw line */
void tek4662_line(int address, int x1, int y1, int x2, int y2) {
    tek4662_move_to(address, x1, y1);
    tek4662_draw_to(address, x2, y2);
}

/* Draw rectangle */
void tek4662_rectangle(int address, int x1, int y1, int x2, int y2) {
    tek4662_move_to(address, x1, y1);
    tek4662_draw_to(address, x2, y1);
    tek4662_draw_to(address, x2, y2);
    tek4662_draw_to(address, x1, y2);
    tek4662_draw_to(address, x1, y1);
}

/* Set text position and write text */
void tek4662_text(int address, int x, int y, const char *text) {
    char us = TEK_US;
    
    /* Enter text mode */
    tek4662_send(address, &us, 1);
    
    /* Position cursor using graphics coordinate */
    tek4662_move_to(address, x, y);
    
    /* Back to text mode and send text */
    tek4662_send(address, &us, 1);
    tek4662_send(address, text, strlen(text));
}

/* Main plotter printing function */
void print_graph_plotter(void) {
    int i, j, x, y;
    float value, y_range;
    int trace_count = 0;
    ...
    float min_trace, max_trace;
    int k;
    int plotter_address = 18;  /* Your Tek 4662 address */
    
    /* Plotter dimensions in TEK coordinates (0-1023) */
    int plot_left = 100;
    int plot_right = 900;
    int plot_top = 800;
    int plot_bottom = 200;
    ...
    printf("\nPrinting to Tektronix 4662 Plotter...\n");
    ...
    /* Put plotter in remote mode */
    gpib_remote(plotter_address);
    delay(200);
    
    /* Clear plotter and initialize */
    cmd[0] = TEK_ESC;
    cmd[1] = TEK_FF;
    cmd[2] = '\0';
    tek4662_send(plotter_address, cmd, 2);  /* ESC FF - Clear */
    delay(1000);  /* Wait for mechanical clear */
    
    /* Enter bypass mode */
    cmd[0] = TEK_ESC;
    cmd[1] = 0x05;  /* CTRL-E */
    cmd[2] = '\0';
    tek4662_send(plotter_address, cmd, 2);
    delay(100);
    
    /* Draw title */
    printf("Drawing title...\n");
    tek4662_text(plotter_address, 300, 950, "TM5000 HIGH-PRECISION MEASUREMENT");
    
    /* Draw scale info */
    if (decimal_places == 0) {
        sprintf(label, "Scale: %.0f to %.0f %s", 
                min_val * scale_factor,
                max_val * scale_factor,
                unit_str);
    } else {
        sprintf(label, "Scale: %.*f to %.*f %s", 
                decimal_places, min_val * scale_factor,
                decimal_places, max_val * scale_factor,
                unit_str);
    }
    tek4662_text(plotter_address, 300, 920, label);
    
    /* Sample rate info */
    sprintf(label, "Sample Rate: %d ms", g_control_panel.sample_rate_ms);
    tek4662_text(plotter_address, 300, 890, label);
    
    /* Draw border */
    printf("Drawing axes and grid...\n");
    tek4662_rectangle(plotter_address, plot_left, plot_bottom, plot_right, plot_top);
    
    /* Draw grid lines - 10 divisions for precision */
    for (i = 1; i < 10; i++) {
        /* Horizontal grid (dashed manually) */
        y = plot_bottom + (plot_height * i) / 10;
        for (x = plot_left; x < plot_right; x += 20) {
            tek4662_line(plotter_address, x, y, x + 10, y);
        }
        
        /* Vertical grid (dashed manually) */
        x = plot_left + (plot_width * i) / 10;
        for (y = plot_bottom; y < plot_top; y += 20) {
            tek4662_line(plotter_address, x, y, x, y + 10);
        }
    }
    
    /* Y-axis labels */
    printf("Drawing Y-axis labels...\n");
    for (i = 0; i <= 10; i++) {
        float y_value;
        y = plot_bottom + (plot_height * i) / 10;
        y_value = min_val + (y_range * i / 10.0);
        
        /* Format label */
        if (decimal_places == 0) {
            sprintf(label, "%.0f", y_value * scale_factor);
        } else {
            sprintf(label, "%.*f", decimal_places, y_value * scale_factor);
        }
        
        /* Position text to left of axis */
        tek4662_text(plotter_address, plot_left - 60, y, label);
    }
    
    /* Y-axis unit label */
    tek4662_text(plotter_address, 20, plot_bottom + plot_height/2, unit_str);
    
    /* X-axis labels */
    printf("Drawing X-axis labels...\n");
    for (i = 0; i <= 10; i++) {
        x = plot_left + (plot_width * i) / 10;
        if (i % 2 == 0) {  /* Label every other tick */
            sprintf(label, "%d", (max_samples * i) / 10);
            tek4662_text(plotter_address, x - 20, plot_bottom - 40, label);
        }
    }
    
    /* X-axis label */
    tek4662_text(plotter_address, plot_left + plot_width/2 - 50, plot_bottom - 80, 
                 "Sample Number");
    
    /* Plot traces */
    printf("Plotting data traces...\n");
    for (i = 0; i < 8; i++) {
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            printf("  Plotting trace %d (%d samples)...\n", i, g_traces[i].data_count);
            
            /* Calculate scales */
            x_scale = (float)plot_width / (float)(g_traces[i].data_count - 1);
            y_scale = (float)plot_height / y_range;
            
            /* Plot the trace */
            for (j = 0; j < g_traces[i].data_count - 1; j++) {
                /* Calculate positions */
                x_pos1 = plot_left + (int)(j * x_scale);
                x_pos2 = plot_left + (int)((j + 1) * x_scale);
                
                /* Invert Y since Tek coordinates have Y=0 at bottom */
                y_pos1 = plot_bottom + (int)((g_traces[i].data[j] - min_val) * y_scale);
                y_pos2 = plot_bottom + (int)((g_traces[i].data[j + 1] - min_val) * y_scale);
                
                /* Clip to plot area */
                if (y_pos1 < plot_bottom) y_pos1 = plot_bottom;
                if (y_pos1 > plot_top) y_pos1 = plot_top;
                if (y_pos2 < plot_bottom) y_pos2 = plot_bottom;
                if (y_pos2 > plot_top) y_pos2 = plot_top;
                
                /* Draw line segment */
                if (j == 0) {
                    tek4662_move_to(plotter_address, x_pos1, y_pos1);
                }
                tek4662_draw_to(plotter_address, x_pos2, y_pos2);
            }
        }
    }
    ...
    y = 150;  /* Start legend position */
    
    tek4662_text(plotter_address, 100, y, "Legend:");
    y -= 30;
    ...
    for (i = 0; i < 8; i++) {
        if (g_traces[i].enabled && g_traces[i].data_count > 0) {
            ...
            /* Draw trace sample line */
            tek4662_line(plotter_address, 100, y + 10, 150, y + 10);
            
            /* Trace info */
            sprintf(label, "Slot %d: %s (%d samples)", 
                    i, g_traces[i].label, g_traces[i].data_count);
            tek4662_text(plotter_address, 160, y + 10, label);
            
            /* Statistics */
            if (decimal_places == 0) {
                sprintf(label, "Min: %.0f %s, Max: %.0f %s, Range: %.0f %s",
                        min_trace * scale_factor, unit_str,
                        max_trace * scale_factor, unit_str,
                        (max_trace - min_trace) * scale_factor, unit_str);
            } else {
                sprintf(label, "Min: %.*f %s, Max: %.*f %s, Range: %.*f %s",
                        decimal_places, min_trace * scale_factor, unit_str,
                        decimal_places, max_trace * scale_factor, unit_str,
                        decimal_places + 1, (max_trace - min_trace) * scale_factor, unit_str);
            }
    ...
    /* Footer info */
    sprintf(label, "Zoom: %.2fx, Auto-scale: %s", 
            g_graph_scale.zoom_factor,
            g_graph_scale.auto_scale ? "ON" : "OFF");
    tek4662_text(plotter_address, plot_left, 30, label);
    
    /* Return plotter to local mode */
    printf("Finishing plot...\n");
    
    /* Exit graphics mode */
    cmd[0] = TEK_US;
    cmd[1] = '\0';
    tek4662_send(plotter_address, cmd, 1);
    
    /* Return to local */
    gpib_local(plotter_address);
    
    printf("Tektronix 4662 plot completed successfully.\n");
    printf("Press any key to continue...");
    getch();
}
void print_report(void) {
...
/* Graph configuration menu - NEW FOR V2.8 */
void graph_config_menu(void) {
    int done = 0;
    int choice;
    char input[20];
    ...
    while (!done) {
        ...
        switch (choice) {
            ...
            case '4':  /* Help */
                ...
                printf("  SHIFT+0-7   - Toggle trace visibility\n\n");
    ...
    /* Store sample range in global for graph display to use */
...
void save_data(void) {
    ...
    /* Write global data (for backward compatibility) */
...
void load_data(void) {
    ...
    if (is_new_format) {
        /* New format with module data */
        printf("Loading enhanced format file...\n");
        
        /* Read global samples count */
        ...
        while (fgets(line, sizeof(line), fp)) {
            ...
            /* Check for slot data header */
            if (sscanf(line, "Slot%d:%u", &slot, &module_count) == 2) {
                if (slot >= 0 && slot < 8 && g_system->modules[slot].enabled) {
        ...
        /* Old format - load as before but also populate module data */
        printf("Loading legacy format file...\n");
        
        /* Read global count */
        fscanf(fp, "Samples: %u\n", &g_system->data_count);
        
        /* Try to read sample rate (may not exist) */
        file_pos = ftell(fp);
        ...
        /* For legacy files, copy global data to first enabled module */
        for (i = 0; i < 8; i++) {
            if (g_system->modules[i].enabled) {
    ...
    /* Sync loaded data to traces for graph and math functions */
    sync_traces_with_modules();
}

/* Synchronize g_traces arrays with module data */
void sync_traces_with_modules(void) {
    int i;
...
/* New function to save settings only */
...
/* New function to load settings only */
...
void math_functions_menu(void) {
    ...
    /* Sync traces with module data to ensure math functions see loaded data */
    sync_traces_with_modules();
...
void auto_scale_graph(void) {
    ...
    /* Quantize for microvolt scales - minimum 5µV increments */
    ...
    /* Quantize for sub-millivolt scales - 0.1mV increments */
    else if (range < 0.005) {  /* Less than 5mV total range */
        float center = (g_graph_scale.max_value + g_graph_scale.min_value) / 2.0;
        float range_mv = range * 1000.0;  /* Convert to mV */
        range_mv = ((int)(range_mv * 10.0 + 0.5)) / 10.0;  /* Round to 0.1mV */
    ...
    /* Quantize for millivolt scales - minimum 1mV increments */
    else if (range < 0.050) {  /* Less than 50mV total range */
        float center = (g_graph_scale.max_value + g_graph_scale.min_value) / 2.0;
        float range_mv = range * 1000.0;  /* Convert to mV */
        range_mv = ((int)(range_mv + 0.5));  /* Round to 1mV */
...
void draw_grid_dynamic(int max_samples) {
    ...
    /* Set flags based on unit type for backward compatibility */
...
void main_menu(void) {
    ...
    while (!done) {
        ...
        printf("\n\n");
        printf("          TM5000 CONTROL SYSTEM v2.8\n");
        printf("          ==========================\n\n");
...
int main(void) {
    ...
    clrscr();
    printf("TM5000 GPIB Control System v2.8\n");
